...
=head2 build_params_pair

Builds a C<FCGI_NameValuePair>.
...
A string of octets containing the C<FCGI_NameValuePair>.

=back

=head2 build_params_pair_header

Builds C<FCGI_NameValuePair> header.
...
A string of octets containing the C<FCGI_NameValuePair> header.
...
=head2 parse_params_pair

Parses a C<FCGI_NameValuePair>.
...
C<$octets> must be greater than or equal to 2 octets in length.
...
Length of C<FCGI_NameValuePair> in octets.

=back

=head2 parse_params_pair_header

Parses a C<FCGI_NameValuePair> header.
...
Length of C<FCGI_NameValuePair> header in octets.
...
=head2 compute_params_pair_length
...
=head2 compute_params_pair_header_length
...
Length of C<FCGI_Record> in octets.
...
=item B<(F)> Argument "%s" is not a hash reference

=item B<(F)> Argument "%s" is not an unsigned integer

=item B<(F)> Argument "%s" is not an unsigned %u-bit integer
...
=item B<(F)> Argument "%s" is outside of octets length

=item B<(F)> Unexpected end of octets while parsing FCGI_NameValuePair header

=item B<(F)> Unexpected end of octets while parsing FCGI_NameValuePair name

=item B<(F)> Unexpected end of octets while parsing FCGI_NameValuePair value
