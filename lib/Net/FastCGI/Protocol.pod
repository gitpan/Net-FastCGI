...
A hash reference containing the keys C<type> and C<request_id> and
following depending on the record type:
...
Returns a hash reference containing the C<FCG_Record> components.
See L</parse_record>.
